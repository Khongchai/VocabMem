...
const UserResponse_1 = __importDefault(require("./UserResponse"));
let UserResolver = class UserResolver {
    ...
    login(usernameOrEmail, password, { req }) {
        return __awaiter(this, void 0, void 0, function* () {
            let user = yield User_1.User.findOne(usernameOrEmail.includes("@")
                ? { where: { email: usernameOrEmail } }
                : { where: { username: usernameOrEmail.toLowerCase() } });
            const errors = yield validateLogin_1.default(user, password);
            if (errors) {
                return errors;
            }
            else {
...
__decorate([
    type_graphql_1.Mutation(() => UserResponse_1.default),
